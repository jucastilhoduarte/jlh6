...
public class KernelShellTest {
    ...
    static final String APPLY =
        ...
        + "&& iptables -t nat -C PREROUTING -i wlan2 -p udp --dport 53 -j DNAT --to-destination 8.8.8.8:53 2>/dev/null "
        + "&& iptables -t nat -C PREROUTING -i wlan2 -p tcp --dport 53 -j DNAT --to-destination 8.8.8.8:53 2>/dev/null "
    ...
    static final String PURGE =
        "while ip rule | grep -q 'iif wlan2 lookup main'; do ip rule del from all iif wlan2 lookup main suppress_prefixlength 0 priority 17998 2>/dev/null || break; done; "
        + "while ip rule | grep -q 'iif wlan2 lookup wlan0'; do ip rule del from all iif wlan2 lookup wlan0 priority 17999 2>/dev/null || break; done; "
        + "while iptables -t nat -C PREROUTING -i wlan2 -p udp --dport 53 -j DNAT --to-destination 8.8.8.8:53 2>/dev/null; do iptables -t nat -D PREROUTING -i wlan2 -p udp --dport 53 -j DNAT --to-destination 8.8.8.8:53 2>/dev/null || break; done; "
        + "while iptables -t nat -C PREROUTING -i wlan2 -p tcp --dport 53 -j DNAT --to-destination 8.8.8.8:53 2>/dev/null; do iptables -t nat -D PREROUTING -i wlan2 -p tcp --dport 53 -j DNAT --to-destination 8.8.8.8:53 2>/dev/null || break; done; "
