...
- No boot: `restoreIfEnabled` só entra STARTING se `enabled=true` **e** `autostart=true` (autostart default OFF; sem ele o router só liga por tap manual); nunca aplica regras sem ping OK verificado; o monitor de recovery rearma sozinho ao chegar em ACTIVE (sempre ON)
